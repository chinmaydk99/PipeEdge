...
def _make_shard(model_name, model_file, stage_layers, stage, q_bits, prune):
    shard = model_cfg_apt.module_shard_factory(model_name, model_file, stage_layers[stage][0],
...
def evaluation(args, dataset_cfg):
    ...
    if prune:
        pruned_model_file = model_cfg_apt._MODEL_CONFIGS[model_name]['pruned_weights_file']
        # dataset_split = 'train'
        ...
        for ubatch, ubatch_labels in train_loader:
            config = model_cfg_apt.get_model_config(model_name)
            shard_config = model_cfg_apt.ModuleShardConfig(layer_start=1, layer_end=model_cfg_apt.get_model_layers(model_name),
                                            is_first=True, is_last=True)
            model_file = model_cfg_apt.get_model_default_weights_file(model_name)
            
            model = model_cfg_apt._MODEL_CONFIGS[model_name]['shard_module'](config, shard_config, model_file)
            ...
            with redirect_stdout(output_buffer):
                if calibrate:
                    ...
                    elif prune_method == 'apt_ss':
                        # APT pruning with Solution S for mask selection and Solution S for compensation
                        print(f"Using APT-SS pruning with keep_ratio = {keep_ratio} followed by calibration")
                        weights = model.prune_apt_and_calibrate(
                            ubatch,
                            calib_loader=calib_loader,
                            keep_ratio=keep_ratio,
                            solution_mask='s',
                    ...
                    elif prune_method == 'apt_sm':
                        # APT pruning with Solution S for mask selection and Solution M for compensation
                        print(f"Using APT-SM pruning with keep_ratio = {keep_ratio} followed by calibration")
                        weights = model.prune_apt_and_calibrate(
                            ubatch,
                            calib_loader=calib_loader,
                            keep_ratio=keep_ratio,
                    ...
                    if prune_method == 'wanda':
                        ...
                        # APT pruning with Solution S for mask and Solution S for compensation
                        print(f"Using APT-SS pruning with keep_ratio = {keep_ratio}")
                        weights = model.prune_apt_ss(ubatch, keep_ratio=keep_ratio, dampening=args.dampening)
                    elif prune_method == 'apt_sm':
                        # APT pruning with Solution S for mask and Solution M for compensation
                        print(f"Using APT-SM pruning with keep_ratio = {keep_ratio}")
                        weights = model.prune_apt_sm(ubatch, keep_ratio=keep_ratio, dampening=args.dampening)
                    elif prune_method == 'apt_iterative':
                        # Iterative APT pruning
                        print(f"Using iterative APT pruning with final_keep_ratio = {keep_ratio}, steps = {iterative_steps}")
                        mini_test_batch = None
                        try:
                            val_iter = iter(val_loader)
                            mini_test_batch = next(val_iter)
                        ...
                        weights = model.prune_apt_iterative(
                            ubatch, 
                            final_keep_ratio=keep_ratio, 
...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", "--model-name", type=str, default="google/vit-base-patch16-224",
                        choices=model_cfg_apt.get_model_names(),
    ...
    dset.add_argument("--prune-method", type=str, default="wanda", choices=["wanda", "iterative", "apt_ss", "apt_sm", "apt_iterative"],
                      help="Pruning method to use (wanda=WANDA one-shot, iterative=WANDA iterative, apt_ss=APT with Solution S-S, apt_sm=APT with Solution S-M, apt_iterative=Iterative APT)")
