...
def evaluation(args, dataset_cfg):
    ...
    if prune:
        ...
        for ubatch, ubatch_labels in train_loader:
            ...
            shard_config = model_cfg_dsnot.ModuleShardConfig(layer_start=1, layer_end=model_cfg_dsnot.get_model_layers(model_name),
                                            is_first=True, is_last=True)
            model_file = model_cfg_dsnot.get_model_default_weights_file(model_name)
            
            model = model_cfg_dsnot._MODEL_CONFIGS[model_name]['shard_module'](config, shard_config, model_file)
            
            # Capture the density outputs during pruning
            output_buffer = io.StringIO()
            
            # Store original dense weights for DSnoT if requested
            original_weights = None
            if refine_dsnot:
                print("Saving original dense weights for DSnoT refinement...")
            ...
            # Apply DSnoT refinement if requested
            if refine_dsnot:
                print("\n===== Applying DSnoT Refinement =====")
                # Load the pruned weights into the model to ensure correct state
                model.load_state_dict(weights)
                
                # Apply DSnoT refinement
                refined_weights = model.refine_dsnot(
                    ubatch, 
                ...
                # Replace original pruned weights with refined weights
                weights = refined_weights
                
                # Capture updated sparsity information
                output_buffer = io.StringIO()
                with redirect_stdout(output_buffer):
                    # Load refined weights for sparsity reporting
                    model.load_state_dict(weights)
                    # Print layer information for capturing
                    for name, module in model.named_modules():
                        if isinstance(module, torch.nn.Linear):
                            W = module.weight.data
                            density = (W != 0).sum().item() / W.numel()
                            print(f"Layer:{name} => Density: {density:.4f}")
                
                # Process captured output again for refined model
                for line in output_buffer.getvalue().split('\n'):
                    if "Layer:" in line and "Density:" in line:
                        acc_reporter.capture_sparsity(line)
...
if __name__ == "__main__":
    ...
    # DSnoT Refinement arguments - keep but match structure to others
    dsnot = parser.add_argument_group('DSnoT Refinement arguments')
    dsnot.add_argument("--refine-dsnot", type=bool, nargs='?', const=True, default=False,
                      help="Whether to apply Dynamic Sparse No Training refinement after pruning")
    dsnot.add_argument("--dsnot-cycles", type=int, default=50,
                      help="Maximum number of DSnoT refinement cycles per layer")
    dsnot.add_argument("--dsnot-error-threshold", type=float, default=0.1,
                      help="Error threshold for early stopping in DSnoT")
    dsnot.add_argument("--dsnot-var-power", type=float, default=1.0,
                      help="Power of variance term in DSnoT growing criterion")
